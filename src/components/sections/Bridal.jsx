  ...
  return (
    <section className="relative py-24 overflow-hidden">
      ...
      <div className="absolute inset-0">
        <img src={BRIDAL_BG} alt="Bridal Makeup" className="w-full h-full object-cover" loading="lazy" />
        <div
          className="absolute inset-0"
          style={{
            background:
              'linear-gradient(135deg, rgba(0,0,0,0.92) 0%, rgba(10,8,4,0.82) 50%, rgba(0,0,0,0.94) 100%)',
          }}
        />
        <div
          className="absolute inset-0"
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <ScrollReveal className="text-center mb-14">
          <SectionBadge dark>{bridal.badge}</SectionBadge>
          <h2 className="font-display font-bold text-4xl sm:text-5xl mt-3 mb-3 text-white leading-tight">
            {bridal.heading}
          </h2>
          <div className="gold-divider max-w-xs mx-auto my-5" />
          <p className="text-white/60 font-body text-lg max-w-2xl mx-auto">{bridal.subheading}</p>
        </ScrollReveal>
        ...
        <StaggerContainer className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5" staggerChildren={0.1}>
          {bridal.services.map(({ title, desc }) => (
            <motion.div
              ...
              style={{
                background:
                  'linear-gradient(145deg, rgba(201,162,78,0.08) 0%, rgba(10,10,10,0.6) 100%)',
              }}
